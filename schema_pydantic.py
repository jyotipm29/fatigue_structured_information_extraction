...
class LoadingTypeValue(str, Enum):
    """
    Types of loading conditions.
    """
    axial = "axial"
    bending = "bending"
    rotating_bending = "rotating_bending"
    shearing = "shearing"
    torsion = "torsion"
    unknown = "unknown"


class ControlTypeValue(str, Enum):
    """
    Methods of controlling the loading application.
    """
    strain_controlled = "strain_controlled"
    force_controlled = "force_controlled"
    displacement_controlled = "displacement_controlled"
    unknown = "unknown"


class WaveformTypeValue(str, Enum):
    """
    Types of applied loading waveform shapes.
    """
...
class AxisScaleValue(str, Enum):
    """
    Scale types for plotting data axes.
    """
    linear = "linear"
    logarithmic_base_10 = "logarithmic_base_10"



class FatigueTestSeries(ConfiguredBaseModel):
    """
    Represents a single series of fatigue tests with testing parameters and results.
    """
    series_reference_from_text: str = Field(..., description="""A string identifier (or label) uniquely representing a single fatigue test series, as referenced explicitly in the narrative text.  This identifier should correspond to a series for which experimentally obtained data is presented in the form of an S-N (stress-life)  or e-N (strain-life) curve. An S-N diagram plots stress against the number of cycles to failure, while an e-N diagram  plots strain against the number of cycles to failure.
...
If symbols are present in an identified label, they are converted to LaTeX math notation (enclosed in $...$), except for degree  Celsius (°C). The extracted label should be concise yet sufficient to uniquely identify its corresponding test series as cited  in the text.""")
    series_reference_from_image: str = Field(..., description="""A string identifier (or label) uniquely representing a single fatigue test series depicted in an image containing an  S-N (stress-life) or e-N (strain-life) diagram. An S-N diagram plots stress against the number of cycles to failure,  while an e-N diagram plots strain against the number of cycles to failure.
...
Ambiguous references such as \"room temperature\" are not treated as valid numerical values unless a value  (e.g., 20°C, 25°C, or 298 K) is explicitly provided alongside.""")
    test_temperature_descriptor: str = Field(..., description="""The test temperature descriptor is a rather vague string description of the temperature under which a test is  being performed, e.g. ambient room temperature.""")
    test_temperature_unit: str = Field(..., description="""The unit in which the test temperature is reported. Standard unit of temperature in the International System of  Units (SI) is Kelvin (K) but it is also commonly reported in degree Celsius (°C).""")
    ratio_r_value: float = Field(..., description="""A unitless floating point number that represents the ratio of minimum to maximum within a cycle of a loading waveform.  The loading waveform constrains a single physical quantity — either stress, strain, or load/force. The ratio must not  mix quantities (e.g., stress and strain), and should be interpreted as one of:
...
- If the loading_type is 'rotating_bending', then the ratio_r_value is always -1.""")
    ratio_r_descriptor: str = Field(..., description="""A brief, textual description of the loading condition corresponding to the ratio_r_value. Examples include: \"tension-tension\", \"pulsating\", \"partly reversed tension-compression\",  \"fully-reversed\", \"partly reversed compression\", \"compression pulsating\" , \"compression-compression\", etc.
If a descriptor is provided that clearly maps to a known ratio_r_value (i.e, \"fully-reversed\" implies  ratio_r_value = -1 and \"pulsating\" implies ratio_r_value = 0), the corresponding numerical value for ratio_r_value  can be inferred.""")
    mean_stress_value: float = Field(..., description="""The mean stress S_m is the averaged value of the mechanical stresses during fatigue testing (S_m = (S_min + S_max)/2).
Inference Notes: - If the ratio_r_value is -1 and the control type is 'force_controlled', then the mean stress value is always 0. - If the ratio_r_value is -1 and the control type is 'strain_controlled', then the mean stress value cannot be inferred
  without additional context.""")
    mean_stress_unit: str = Field(..., description="""The unit of the mean stress S_m. MegaPascal (MPa), GigaPascal (GPa), or kilopounds per square inch (ksi) are typical  for fatigue testing. If the mean stress value is inferred as 0, its unit can be assumed to align with the units used for other reported  stress values in the context.""")
    stress_concentration_factor_value: float = Field(..., description="""The stress concentration factor K_t is a unitless float variable which expresses the ratio of the highest stress  (dictated by a present notch) to a nominal far-field stress of the cross section (K_t = S_max/S_nominal).""")
    cycle_limit_value: float = Field(..., description="""The number of load cycles at which the test is stopped, and the specimen is considered a \"run-out\", meaning no  failure has occurred by the end of the test. If not explicitly stated, it can be inferred from the S-N (stress-life)  curve plot. If all run-out samples cluster around a particular number of cycles in the plot, that number can be  considered as the cycle_limit_value. The run-outs are typically represented with a rightward-pointing arrow in the plot.""")
    fatigue_strength_at_cycle_limit_value: float = Field(..., description="""Fatigue strength at the cycle limit refers to the maximum cyclic stress amplitude a material can withstand for a predetermined  number of load cycles (the cycle limit) before failure occurs. For example, if a material has a fatigue strength of  200 MPa at 10^6 cycles, this means that when subjected to a cyclic stress of 200 MPa, the material is expected to fail  after approximately 10^6 cycles.""")
    fatigue_strength_at_cycle_limit_unit: str = Field(..., description="""The unit in which the fatigue strength at the cycle limit is reported. MegaPascal (MPa), GigaPascal (GPa), or  kilopounds per square inch (ksi) are typical for fatigue testing.""")
    fatigue_endurance_limit_value: float = Field(..., description="""The fatigue endurance limit (or fatigue limit) is the maximum cyclic stress amplitude that a material can withstand without  failure, assuming an infinite number of load cycles. 
...
Note: These are some of the common representations, but other forms may exist depending on the context or source. When the  value is inferred based on these formulations, it should be expressed with a precision of up to 6 decimal places.""")
    sn_curve_x_axis_label: str = Field(..., description="""The label for the X-axis of the S-N (stress-life) or e-N (strain-life) curve plot, typically representing either  the number of load cycles to failure or the logarithm of the number of cycles.""")
    sn_curve_x_axis_scale: AxisScaleValue = Field(..., description="""Specifies the scale of the X-axis for the S-N (stress-life) or e-N (strain-life) curve plot. The scale can be linear,  with equal spacing between the ticks, or logarithmic, characterized by uneven spacing between the ticks, especially at  higher values.""")
    sn_curve_y_axis_label: str = Field(..., description="""The label for the Y-axis of the S-N (stress-life) or e-N (strain-life) curve plot, typically representing either  the stress amplitude or range (for S-N) or strain amplitude or range (for e-N).""")
    sn_curve_y_axis_scale: AxisScaleValue = Field(..., description="""Specifies the scale of the Y-axis for the S-N (stress-life) or e-N (strain-life) curve plot. The scale can be linear,  with equal spacing between the ticks, or logarithmic, characterized by uneven spacing between the ticks, especially at  higher values.""")
    failure_criterion: str = Field(..., description="""A failure criterion is a concise string that provides a threshold on whether a specimen is failed or not. For instance, in a load-controlled test, a load drop of 3 or 5% is used as an indication for the specimen being failed. Other times, a sample is only considered failed once complete rupture occurred. In resonance testing, often a relative resonance frequency drop is used as a failure criterion. Test stoppage at a predefined number of cycles, or run-outs, are not considered failure criteria.""")
    fatigue_testing_machine_name: str = Field(..., description="""A string identifier specifying the name or model of the machine used for conducting fatigue tests.  The name should exclude any descriptive phrases (e.g., 'fatigue testing machine') and focus solely on the brand, model,  and/or machine name. Any hyphenated or multi-part names should be preserved in their original format.""")
    fatigue_testing_machine_class: str = Field(..., description="""The fatigue testing machine class refers to a concise, technical term that describes the type of fatigue testing machine based  solely on its functioning principle or core technology. Full equipment names, generic phrases, or unnecessary descriptors—for example,  \"fatigue testing machine\" should not appear in the class name. Examples include: \"resonance\", \"servo-hydraulic\", \"ultrasonic\",  \"piezoelectric ultrasonic\", \"rotating bending\", \"rotating bar two-point bending\", etc.""")


class Container(ConfiguredBaseModel):
    """
    Root container class holding a collection of fatigue test series.
    """
    fatigue_test_series_list: List[FatigueTestSeries] = Field(..., description="""List of all fatigue test series.""")


# Model rebuild
# see https://pydantic-docs.helpmanual.io/usage/models/#rebuilding-a-model
FatigueTestSeries.model_rebuild()
